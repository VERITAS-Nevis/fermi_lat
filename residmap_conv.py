...
def residmap(gta, prefix='', convolve=True, **kwargs):
    ...
    gta.logger.info('Generating residual maps')

    schema = ConfigSchema(gta.defaults['residmap'])

    config = schema.create_config(gta.config['residmap'], **kwargs)
    ...
    o = _make_residual_map(gta, prefix, convolve, **config)
    ...
    gta.logger.info('Finished residual maps')

    outfile = utils.format_filename(gta.workdir, 'residmap',
                                    prefix=[o['name']])

    if config['write_fits']:
        o['file'] = os.path.basename(outfile) + '.fits'
        gta._make_residmap_fits(o, outfile + '.fits')

    if config['write_npy']:
        np.save(outfile + '.npy', o)

    gta.logger.info('Execution time: %.2f s', timer.elapsed_time)
    return o

def _make_residmap_fits(gta, data, filename, **kwargs):
...
def _make_residual_map(gta, prefix, convolve, **kwargs):

    if gta.projtype == 'HPX':
        return _make_residual_map_hpx(gta, prefix, convolve, **kwargs)
    elif gta.projtype == "WCS":
        return _make_residual_map_wcs(gta, prefix, convolve, **kwargs)
    else:
        raise Exception(
            "Did not recognize projection type %s", gta.projtype)

def _make_residual_map_wcs(gta, prefix, convolve, **kwargs):
    ...
    if loge_bounds:
        ...
        loge_bounds[0] = (loge_bounds[0] if loge_bounds[0] is not None
                          else gta.log_energies[0])
        loge_bounds[1] = (loge_bounds[1] if loge_bounds[1] is not None
                          else gta.log_energies[-1])
    else:
        loge_bounds = [gta.log_energies[0], gta.log_energies[-1]]

        
    mmst = np.zeros(gta.npix[::-1])
    cmst = np.zeros(gta.npix[::-1])
    emst = np.zeros(gta.npix[::-1])
    
    ts = np.zeros(gta.npix[::-1])
    sigma = np.zeros(gta.npix[::-1])
    excess = np.zeros(gta.npix[::-1])
    
    geom = gta.geom.to_image()
    ...
    if convolve:
        ...
        xpix, ypix = (np.round((gta.npix[0] - 1.0) / 2.),
                      np.round((gta.npix[1] - 1.0) / 2.))
        ...
        src = gta.roi.get_source_by_name('residmap_testsource')
...
def _make_residual_map_hpx(gta, prefix, convolve, **kwargs):
    exclude = kwargs.setdefault('exclude', None)
    loge_bounds = kwargs.setdefault('loge_bounds', None)
    ...
    hpxsky = gta.counts_map().geom.to_image()
    ...
    if convolve:
        ...
        for c in comp:
            ...
            cms = cc.sum_over_axes()
            mms = mc.sum_over_axes()
            ems = ec.sum_over_axes()
